...
- **4.2.1. Bounded Context: Inventory and Telemetry**
 ...
 - **4.2.1.2. Interface Layer**
      ...
      En la Interface Layer del bounded context Inventory and Telemetry se implementan los controladores, recursos y ensambladores que gestionan la comunicación entre los clientes externos (App Móvil, App Web y Hardware IoT) y la lógica de dominio. Esta capa expone los endpoints REST necesarios para registrar dispensadores, consultar el estado del stock en tiempo real y recibir las lecturas de telemetría enviadas por el microcontrolador.
      ...
      **Sub-capa REST:**
      ...
      | Controller  | TelemetryController                 | Gestiona las solicitudes HTTP de entrada de datos desde el hardware IoT.                             | Recibir las lecturas de los sensores enviadas por el microcontrolador vía Wi-Fi y procesarlas.                | Usa `TelemetryResource` y `TelemetryIngestionService`.    |
      ...
      | Resource    | TelemetryResource                   | Estructura JSON que recibe los datos crudos del hardware (peso, nivel, flujo).                       | Representar la lectura de sensores enviada por el microcontrolador en un formato que el sistema comprenda.   | Usado por `TelemetryController`.                          |
      ...
      | Assembler   | TelemetryFromResourceAssembler      | Convierte el `TelemetryResource` recibido en un comando `ProcessTelemetryCommand`.                   | Prevenir inconsistencias al transformar los datos crudos del hardware a comandos de la Application Layer.    | Usado por `TelemetryController`.                          |
   - **4.2.1.3. Application Layer**
      ...
      **Sub-capa Internal:**
      ...
      | CommandHandler  | UpdateDispenserStatusHandler    | Ejecuta la actualización del estado operativo de un dispensador cuando el hardware reporta un cambio (ej. batería baja, sensor desconectado).            | Modificar el `DispenserStatus` del aggregate `Dispenser` y persistir el cambio en el repositorio.                             | Usa `DispenserRepository`.                                                |
     ...
     - **4.2.1.4. Infrastructure Layer**
      ...
      En la Infrastructure Layer del bounded context Inventory and Telemetry se implementan los componentes técnicos que soportan la persistencia de datos, la comunicación con el hardware IoT y la traducción de señales físicas al lenguaje del dominio. Esta capa es la única que conoce los detalles tecnológicos del sistema, como el protocolo de comunicación del microcontrolador, el motor de base de datos utilizado y la implementación concreta de los repositorios definidos en el dominio.
      ...
      **Sub-capa Repository:**
      ...
      | Class     | StockReadingRepositoryImpl | Implementación concreta de `StockReadingRepository` conectada al motor de base de datos del sistema. | Ejecutar las operaciones de persistencia de cada lectura de sensores capturada por el hardware.            | Usada por `ProcessTelemetryCommandHandler`.           |

      **Sub-capa External Services:**

      | Tipo  | Nombre                        | Descripción                                                                                                                                     | Responsabilidad Principal                                                                                                          | Relación con otros elementos                             |
      |-------|-------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------|
      | Class | MqttHardwareAdapter           | Implementación del protocolo MQTT para recibir los mensajes enviados por el microcontrolador del dispensador vía Wi-Fi.                         | Escuchar el tópico MQTT del dispositivo, deserializar el payload recibido y traducirlo a un `TelemetryResource` para el sistema.  | Invoca a `TelemetryController` con los datos procesados. |
      | Class | SensorDataAntiCorruptionLayer | Capa de traducción que convierte las señales crudas y heterogéneas de los tres sensores al modelo `SensorData` del dominio.                     | Proteger al dominio de cambios en los protocolos o formatos de datos del hardware, actuando como barrera entre el mundo físico y el digital. | Usada por `MqttHardwareAdapter` antes de invocar al controlador. |
